...
contract EscrowDisputesTest is Test {
    ...
    function _createEscrowWithArbitrator() internal returns (uint256) {
        ...
        uint256 escrowId = escrow.createEscrow(freelancer, arbitrator, address(token), ESCROW_AMOUNT, deadline);
    ...
    function _createEscrowWithoutArbitrator() internal returns (uint256) {
        ...
        uint256 deadline = block.timestamp + DEADLINE_OFFSET;
        uint256 escrowId = escrow.createEscrow(freelancer, address(0), address(token), ESCROW_AMOUNT, deadline);
    ...
    function test_raiseDispute_ByClient_Success() public {
        ...
        assertEq(uint256(data.state), uint256(IEscrow.EscrowState.Disputed));
    ...
    function test_raiseDispute_ByAttacker_Reverts() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IEscrow.OnlyPartiesCanRaiseDispute.selector, attacker));
        vm.prank(attacker);
    ...
    function test_raiseDispute_AlreadyDisputed_Reverts() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IEscrow.InvalidState.selector, IEscrow.EscrowState.Disputed, IEscrow.EscrowState.Funded
            )
        );
        vm.prank(freelancer);
    ...
    function test_raiseDispute_BlocksMilestoneRelease() public {
        ...
        uint256 escrowId = escrow.createEscrowWithMilestones(freelancer, arbitrator, address(token), milestones);
        vm.stopPrank();
    ...
    function test_resolveDispute_FullAmountToFreelancer_Success() public {
        ...
        assertEq(token.balanceOf(client), clientBalBefore, "Client should receive nothing");

        IEscrow.EscrowData memory data = escrow.getEscrow(escrowId);
        assertEq(uint256(data.state), uint256(IEscrow.EscrowState.Resolved));
    }
    ...
    function test_resolveDispute_FullAmountToClient_Success() public {
        ...
        vm.prank(arbitrator);
        escrow.resolveDispute(
            escrowId, client, availableAfterFee, "Work does not match specifications. Full refund warranted."
        );
        ...
        assertEq(
            token.balanceOf(arbitrator), arbitratorBalBefore + arbitrationFee, "Arbitrator should get 2% of net escrow"
        );
        assertEq(token.balanceOf(freelancer), freelancerBalBefore, "Freelancer should receive nothing");
    }
    ...
    function test_resolveDispute_PartialAmount_Success() public {
        ...
        assertEq(token.balanceOf(freelancer), freelancerBalBefore + freelancerAmount, "Freelancer should get 60%");
        assertEq(token.balanceOf(client), clientBalBefore + clientAmount, "Client should get 40% refund");
        assertEq(
            token.balanceOf(arbitrator),
            arbitratorBalBefore + arbitrationFee,
        ...
        assertEq(freelancerAmount + clientAmount + arbitrationFee, netInEscrow, "All funds should be distributed");
    }
    ...
    function test_resolveDispute_OnlyArbitrator_Reverts() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IEscrow.NotAuthorized.selector, client, arbitrator));
        vm.prank(client);
        escrow.resolveDispute(escrowId, freelancer, amount, "Client favors freelancer");

        vm.expectRevert(abi.encodeWithSelector(IEscrow.NotAuthorized.selector, freelancer, arbitrator));
        vm.prank(freelancer);
        escrow.resolveDispute(escrowId, freelancer, amount, "Freelancer favors themselves");

        vm.expectRevert(abi.encodeWithSelector(IEscrow.NotAuthorized.selector, attacker, arbitrator));
        vm.prank(attacker);
    ...
    function test_resolveDispute_NoActiveDispute_Reverts() public {
        ...
        vm.prank(arbitrator);
        escrow.resolveDispute(escrowId, freelancer, ESCROW_AMOUNT, "Trying to resolve without dispute");
    }
    ...
    function test_resolveDispute_ExcessiveAmount_Reverts() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(IEscrow.InsufficientFundsForResolution.selector, excessiveAmount, availableAfterFee)
        );
        vm.prank(arbitrator);
        escrow.resolveDispute(escrowId, freelancer, excessiveAmount, "Trying to award too much");
    }
    ...
    function test_resolveDispute_ZeroAmount_Reverts() public {
        ...
        vm.prank(arbitrator);
        escrow.resolveDispute(escrowId, freelancer, 0, "Winner gets nothing?");
    }
    ...
    function test_resolveDispute_ArbitratorAsWinner_Reverts() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IEscrow.OnlyPartiesCanRaiseDispute.selector, arbitrator));
        vm.prank(arbitrator);
        escrow.resolveDispute(escrowId, arbitrator, amount, "I'm awarding funds to myself");
    }
    ...
    function test_resolveDispute_EmptyReasoning_Reverts() public {
        ...
        vm.expectRevert(IEscrow.EmptyDisputeReason.selector);
        vm.prank(arbitrator);
        escrow.resolveDispute(escrowId, freelancer, ESCROW_AMOUNT / 2, "");
    }
    ...
    function test_resolveDispute_EmitsEvent() public {
        ...
        vm.prank(arbitrator);
        escrow.resolveDispute(escrowId, freelancer, amount, "50/50 split is fair");
    }
    ...
    function test_disputeAfterPartialMilestoneRelease_Success() public {
        ...
        vm.prank(arbitrator);
        escrow.resolveDispute(escrowId, freelancer, freelancerAmount, "Milestone 3 is 70% complete. Fair split.");

        uint256 totalDistributed = token.balanceOf(freelancer) - freelancerBalBefore + token.balanceOf(client)
            - clientBalBefore + arbitrationFee;
    ...
    function test_fullDisputeWorkflow_Success() public {
        ...
        assertEq(token.balanceOf(freelancer) - freelancerBalBefore, freelancerAmount, "Freelancer should get 90%");
        assertEq(token.balanceOf(arbitrator) - arbitratorBalBefore, arbitrationFee, "Arbitrator should get 2%");
        assertGt(token.balanceOf(client), clientBalBefore, "Client should get some refund");

        uint256 totalDistributed = (token.balanceOf(freelancer) - freelancerBalBefore)
            + (token.balanceOf(client) - clientBalBefore) + (token.balanceOf(arbitrator) - arbitratorBalBefore);
    ...
    function testFuzz_resolveDispute_MathAlwaysAddsUp(uint256 winnerPercentage) public {
        ...
        vm.prank(arbitrator);
        escrow.resolveDispute(escrowId, freelancer, winnerAmount, "Fuzz test resolution");
